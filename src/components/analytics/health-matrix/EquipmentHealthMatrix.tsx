...
const EquipmentHealthMatrix = () => {
  ...
  const checkIfTruncated = (name: string) => {
    ...
    if (truncatedLocations[name] !== isTruncated) {
      setTruncatedLocations(prev => ({
        ...prev,
        [name]: isTruncated
      }));
    }
  ...
  // Create a truncated version of a long name with ellipsis
    ...
    <div className="w-full overflow-auto">
      <TooltipProvider>
        <Table className="min-w-[650px]">
          ...
          <TableBody>
            {healthData.map((item) => {
              const isTruncated = checkIfTruncated(item.location);
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <span className="font-medium">
                            {getTruncatedName(item.location)}
                          </span>
                        </TooltipTrigger>
